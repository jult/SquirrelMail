 ...
 && sqGetGlobalVar('import_list', $import_list, SQ_FORM) && !empty($import_list))
{
   ...
   if (empty($aa_abook))
   {
      ...
      $aa_abook = addressbook_init(FALSE, $no_ldap_for_user_input);
      $aa_abook_list = NULL;
   }
...
else
{
   ...
   //
   $addresses = aa_find_addresses('AA_MODE_USER_INPUT', $message, $mailbox, $passed_id);



   // get a list that is ready for import
   //
   $addresses = build_address_import_list('AA_MODE_USER_INPUT', $addresses);
