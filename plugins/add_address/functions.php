...
function add_address_auto_add_do($args)
{
   ...
   if (check_sm_version(1, 5, 2))
      ...
      $message = $args[2];
   $addresses = aa_find_addresses('AA_MODE_AUTO_ADD', $message, NULL, NULL, array('To', 'Cc', 'Bcc'));
   $addresses = build_address_import_list('AA_MODE_AUTO_ADD', $addresses, FALSE);
   ...
   if (empty($aa_abook))
   {
      ...
      $aa_abook = addressbook_init(FALSE, TRUE);
      $aa_abook_list = NULL;
   }
...
function add_address_link_do(&$links)
{
   ...
   global $message, $passed_id, $dynamic_address_book_link;
   sqgetGlobalVar('mailbox',   $mailbox,   SQ_FORM);

   add_address_init();

   // if no new addresses are in this message, no link is needed
   // (unless we skip this to save server load)
   //
   if ($dynamic_address_book_link)
   {
      $new_addresses = aa_find_addresses('AA_MODE_ADD_LINK', $message, $mailbox, $passed_id);
      if (empty($new_addresses)) return;
   }

   $passed_ent_id = 0;

   // oops, not sanitizing these (as already done by src/read_body.php)
   // opens us up to some low-risk XSS (attacker needs an already-authenticated
   // user to click on a link the attacker has created with a valid message
   // UID in it, which is somewhat unlikely)...  so let's just use the
   // values that the core already sanitized for us
   // 
   ...
   global $passed_ent_id, $startMessage, $account;
...
/**
  ...
  *
  * @param string $context Must be one of three strings: "AA_MODE_AUTO_ADD",
  *                        "AA_MODE_ADD_LINK" or "AA_MODE_USER_INPUT", which
  *                        indicate what context this function is being used
  *                        in (in order to allow context-dependent configuration)
  ...
  *
  */
function aa_find_addresses($context, $message, $mailbox=NULL,
                           $passed_id=NULL, $headers='', $no_ident=NULL)
{
   ...
   if (!is_null($mailbox) && !is_null($passed_id) && $aa_search_body)
   {
      ...
      //
      preg_match_all('/' . $email_regex . '/i', $messagebody, $addresses, PREG_PATTERN_ORDER);
...
//sm_print_r('ALL ADDRESSES:', $addresses);
   ...
   if (empty($aa_abook))
   {
      if ($context == 'AA_MODE_AUTO_ADD')
      {
         global $no_ldap_for_auto_add;
         $no_ldap = $no_ldap_for_auto_add;
      }
      else if ($context == 'AA_MODE_ADD_LINK')
      {
         global $no_ldap_for_add_link;
         $no_ldap = $no_ldap_for_add_link;
      }
      else if ($context == 'AA_MODE_USER_INPUT')
      {
         global $no_ldap_for_user_input;
         $no_ldap = $no_ldap_for_user_input;
      }

      include_once(SM_PATH . 'functions/addressbook.php');
      $aa_abook = addressbook_init(FALSE, $no_ldap);
      $aa_abook_list = NULL;
   }
...
/**
  ...
  *
  */
function build_address_import_list($context, $addresses, $dns_check=NULL)
{
